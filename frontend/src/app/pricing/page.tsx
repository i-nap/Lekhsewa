        ...
        <main className="flex flex-col items-center min-h-screen p-4 pt-24 pb-24 space-y-16">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 w-full max-w-6xl px-4">
                {tiers.map((tier) => (
                    <div
                        key={tier.name}
                        className={`flex flex-col p-8 rounded-2xl border ${tier.highlighted
                    ...
                    >
                        ...
                        <SpotifyButton
                            onClick={() => tier.price === 'Custom' ? window.location.href = '/contact' : loginWithRedirect()}
                            className={`w-full py-3 ${!tier.highlighted ? '!bg-neutral-800 !text-white hover:!bg-neutral-700' : ''}`}
                        >
                            {tier.cta}
                        </SpotifyButton>
                    </div>
